...
import { Checkbox, List, Segmented, Spin, Typography, message } from 'antd';
...
export const SidePanel2 = (props: Props) => {
    ...
    const { projectConceptModel, obcmSnapShotDetails, userInfo } = useAppSelector(state => state.reveloUserInfo)
    ...
    const checkBlockIfHaveData = (id: string) => {
        ...
        if (bill.length === 0) {
            ...
            bill.map((b: any) => {
                if (Object.keys(b.blockInfo[id]).length) {
    ...
    return (<>
        ...
        <div style={{ display: "flex", flexDirection: "row", placeContent: "flex-start", placeItems: "flex-start", minHeight: "50%" }}>
            ...
            <List
                ...
                header={<div style={{ fontSize: "100%", fontWeight: "bolder" }}>Unit ({props.loading ? 0 : props.unit.features.length})</div>}
                bordered
            ...
            <List
                ...
                renderItem={(item: any, index) => {
                    return (
                        ...
                        <List.Item id={item.properties.blocknumber}
                            style={{ display: "flex", flexDirection: "row", placeItems: "center", placeContent: "flex-start" }}>
                            <Checkbox style={{ marginRight: "2%" }} checked={indexesBlock === item.properties.blocknumber} value={indexesBlock} onChange={(e) => {
        ...
        <div style={{ minHeight: "50%", display: "grid", gridTemplateColumns: "50% 50%", gap: "5px", border: "1px solid #d9d9d9", borderRadius: "8px" }}>
            {indexes ? <>  <div style={{ width: "100%", padding: "10px" }}>
                <div style={{ display: "flex", flexDirection: "column", placeItems: "flex-start" }}>
                    <Typography style={{ fontSize: "150%" }}>Unit:</Typography>
                    <Typography style={{ color: "#0075ea", fontSize: "150%" }}>{indexes}</Typography>
                </div>
                <div style={{ display: "flex", flexDirection: "column", placeItems: "flex-start" }}>
                    <Typography style={{ fontSize: "150%" }}>Jurisdictions:</Typography>
                    <div style={{ display: "flex", width: "100%", flexDirection: "row", flexWrap: "wrap" }}>
                        {createEntitySequence(obcmSnapShotDetails.entities, obcmSnapShotDetails.relations).map((el: any, index: number) => {
                            return (
                                <>
                                    <Typography style={{ fontSize: "110%" }} >{el.label}:<b style={{ color: "#0075ea" }}>{unitDetails[el.name]}</b>{index + 1 === obcmSnapShotDetails.entities.length ? "" : ` >`}</Typography >
                                </>)
                        })}
                    </div>
                </div>
                {indexesBlock ? <>
                    <div style={{ display: "flex", flexDirection: "column", placeItems: "flex-start" }}>
                        <Typography style={{ fontSize: "150%" }}>Block:</Typography>
                        <Typography style={{ fontSize: "150%", color: "#0075ea", }}>{indexesBlock}</Typography>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", placeItems: "flex-start" }}>
                        <Typography style={{ fontSize: "150%" }}>Shifts:</Typography>
                        <Segmented options={['Morning', 'Afternoon', 'Evening']} value={value} onChange={(e) => {
                            setValue(e)
                        }} />
                        {shiftIds.map((el: any) => {
                            return (<>
                                {el.shiftName === value ? <>
                                    ...
                                    <AttachmentBox shiftid={el.shiftId} />
                                </> : <>
                                    <Typography>No attachment of this shift</Typography>
                                </>}
                            </>)
                        })}
                    ...
                    <div style={{ display: "flex", flexDirection: "row", placeContent: "center", placeItems: "center", height: "inherit", width: "100%", padding: "20px" }}>
                        <Typography style={{ fontSize: "1.2rem", fontWeight: "bold" }}>Please select block to get details</Typography>
                    </div>

                </>}

            </div></> : <>
                <div style={{ display: "flex", flexDirection: "row", placeContent: "center", placeItems: "center", height: "inherit", width: "100%" }}>
                    <Typography style={{ fontSize: "1.2rem", fontWeight: "bold" }}>Please select unit</Typography>
                </div>
            </>}

            <div style={{ minHeight: "50%", display: "grid", gridTemplateRows: "30% 70%", borderLeft: "1px solid #d9d9d9" }}>
                {bill.length === 0 ? <>
                    <div style={{ height: "100%", display: "flex", flexDirection: "column", placeContent: "center", placeItems: "center", marginBottom: "2px", }}>
                        No data to display
                    </div>
                </> : <>
                ...
                <div style={{ height: "100%", width: "99%", borderTop: "1px solid #d9d9d9", }}>
                    {props.loading ? <>
                        <div style={{ display: "flex", flexDirection: "row", placeContent: "center", placeItems: "center", height: "inherit" }}><Spin /></div>
